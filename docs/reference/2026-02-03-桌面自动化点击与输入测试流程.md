...
以下示例用于理解“点击/输入/断言”的完整闭环；涉及真实桌面交互的 Desktop 命令需要显式 `--dangerous`（下面示例均已添加）：

1) 启动记事本：

```bash
windows-agent desktop launch --name notepad --dangerous
```

2) 等待窗口稳定（仅示例）：

```bash
windows-agent desktop wait --seconds 1 --dangerous
```

3) 在记事本窗口内点击一个大致安全的坐标（你需要按自己的分辨率调整坐标）：

```bash
windows-agent desktop click --x 200 --y 200 --dangerous
```

4) 输入文本：

```bash
windows-agent desktop type --x 200 --y 200 --text \"Windows.Agent Desktop Test\" --clear --dangerous
```

5) 全选复制（通过快捷键），再读取剪贴板用于断言：

```bash
windows-agent desktop shortcut --keys \"ctrl+a\" --dangerous
windows-agent desktop shortcut --keys \"ctrl+c\" --dangerous
windows-agent desktop clipboard --mode paste --dangerous
```
