...
- `Category=Desktop` 已加入 xUnit collection `Desktop` 并禁用并行（避免鼠标/键盘注入互相干扰）。
- `Category=SystemControl` 已加入 xUnit collection `SystemControl` 并禁用并行（避免互相改系统设置）。
...
- `src/Windows.Agent.Test/Desktop/UiaToolTest.cs`（会启动 `Windows.Agent.UiaTestApp` 并通过 UIA 执行 setvalue/invoke）
