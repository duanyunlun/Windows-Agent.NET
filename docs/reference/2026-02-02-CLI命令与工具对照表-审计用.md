...
| `desktop uia-tree` | `Windows.Agent.Tools.Desktop.UiaTool.GetTreeAsync` | 输出指定窗口的 UIA 控件树（限深度） | `windows-agent desktop uia-tree --window \"AppA\" --depth 3 --pretty` | 中：可能泄露窗口/控件文本与结构；输出可能较大。 |
| `desktop uia-find` | `Windows.Agent.Tools.Desktop.UiaTool.FindAsync` | 在指定窗口内按 selector 查找唯一元素 | `windows-agent desktop uia-find --window \"AppA\" --selector \"automationId=btnSend;controlType=Button\" --pretty` | 中：可用于精确定位控件；可能泄露 UI 信息；selector 不唯一会失败并返回部分匹配。 |
| `desktop uia-invoke` | `Windows.Agent.Tools.Desktop.UiaTool.InvokeAsync` | Invoke 模式触发控件（优先于坐标点击） | `windows-agent desktop uia-invoke --window \"AppA\" --selector \"automationId=btnSend;controlType=Button\" --dangerous --pretty` | 高：真实触发按钮行为，可能造成不可逆操作；仍需防误窗/误控件。 |
| `desktop uia-setvalue` | `Windows.Agent.Tools.Desktop.UiaTool.SetValueAsync` | Value 模式设置控件值（优先于坐标输入） | `windows-agent desktop uia-setvalue --window \"AppA\" --selector \"automationId=txtUser;controlType=Edit\" --value \"user\" --dangerous --pretty` | 高：写入输入框/配置字段，可能泄露敏感信息或改坏状态；需确保窗口与控件唯一匹配。 |
