...
### 4.2 UIA2 vs UIA3（FlaUI 后端选择）

在 Windows.Agent 中，UIA 命令支持两套 FlaUI 后端：

- `uia3`（默认）：更适合 Win10/11 现代应用，通常信息/模式更完整。
- `uia2`：在少数“老程序/特殊控件”场景下可能更稳，可作为兼容性回退。

选择方式：在 `desktop uia-*` 命令上增加 `--backend uia3|uia2`。

示例：当 `uia3` 下找不到控件时，尝试 `uia2`：

```bash
windows-agent desktop uia-find --window "AppA" --selector "name=确定;controlType=Button" --backend uia2 --pretty
```

注意：同一个 selector 在 UIA2/UIA3 下可能命中不同数量元素（或属性值不同）。编排器应以“唯一命中”为前提执行 `uia-invoke/uia-setvalue`。
